...
def part1
  ...
  passports.each do |passport|
    attributes = passport.split('\n')
                         .map { |line| line.split(' ') }
                         .flatten
                         .map { |attr| attr.split(':').first }
    valid_passports += 1 if (required_fields - attributes).length == 0
  end
...
def part2
  valid_passports = 0
  required_fields = {
    'byr' => ->(byr) { (byr =~ /^\d\d\d\d$/)&.zero? && byr.to_i >= 1920 && byr.to_i <= 2002 },
    'iyr' => ->(iyr) { (iyr =~ /^\d\d\d\d$/)&.zero? && iyr.to_i >= 2010 && iyr.to_i <= 2020 },
    'eyr' => ->(eyr) { (eyr =~ /^\d\d\d\d$/)&.zero? && eyr.to_i >= 2020 && eyr.to_i <= 2030 },
    'hgt' => ->(hgt) { valid_height(hgt) },
    'hcl' => ->(hcl) { (hcl =~ /#[0-9a-f]{6}/)&.zero? },
    'ecl' => ->(ecl) { %w[amb blu brn gry grn hzl oth].include?(ecl) },
    'pid' => ->(pid) { (pid =~ /^\d\d\d\d\d\d\d\d\d$/)&.zero? }
  }
